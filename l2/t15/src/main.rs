...
fn main() {
    ...
    // 24 байта: сам текст храниться в куче, а функция возращается размер структуры из: указателя - 8 байтов,
    // максимального размера - 8 байтов и текущего размера - 8 байтов
    let size_of_s2 = std::mem::size_of_val(&s2);

    // 16 байтов: так как в данном случае имеется в виду view of String и возращает размер структуры из:
