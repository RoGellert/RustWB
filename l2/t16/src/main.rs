...
fn merge( a: std::sync::mpsc::Receiver<i32>, b: std::sync::mpsc::Receiver<i32>) -> std::sync::mpsc::Receiver<i32> {
    ...
    // цикл в котором поочерёдно читаются и записываются в новый канал данные из двух старых каналов;
...
fn main() {
    // запускает и ждет завершения трэда, возвращая приёмник канала с полученными данными
    let a = as_chan(&vec![1, 3, 5, 7]);

    // запускает и ждет завершения другого трэда, возвращая приёмник канала с полученными данными
    let b = as_chan(&vec![2, 4, 6, 8]);

    // поочередно соединяет два полученных из приёмников каналов массива данных и возвращает новый приёмник
    let c = merge(a, b);
